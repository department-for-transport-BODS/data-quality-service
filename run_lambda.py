...
def main():
    ...
    python run_lambda.py --function_name=last_stop_is_set_down_lambda_handler --file_id=40 --check_id=1 --result_id=8
    """
    parser = argparse.ArgumentParser(description="Run lambda functions manually")
    parser.add_argument("--function_name", help="Add function name to run", default="lambda_handler")
    ...
    functions_to_run[args.function_name](
        event = {
            "Records": [{"body": dumps(obj={"file_id": args.file_id, "check_id": args.check_id, "result_id": args.result_id})}]
        },
        context=None,
    )
