...
class TestMissingJourneyCode(unittest.TestCase):
    ...
    def test_lambda_handler_success(self, mock_logger, mock_check, mock_observation, mock_get_df_vehicle_journey):
        ...
        mocked_check.set_status.assert_called_once_with(DQSTaskResultStatus.SUCCESS.value)

    @patch('src.template.missing_journey_code.get_df_vehicle_journey')
    @patch('src.template.missing_journey_code.ObservationResult')
    @patch('src.template.missing_journey_code.Check')
    @patch('src.template.missing_journey_code.logger')
    def test_lambda_handler_no_null_codes(self, mock_logger, mock_check, mock_observation, mock_get_df_vehicle_journey):
        event = {}
        context = {}
        mocked_check = mock_check.return_value
        mocked_check.validate_requested_check.return_value = True
    ...
    @patch('src.template.missing_journey_code.Check')
    @patch('src.template.missing_journey_code.logger')
    def test_lambda_handler_empty_df(self, mock_logger, mock_check, mock_observation, mock_get_df_vehicle_journey):
        event = {}
        context = {}
        mocked_check = mock_check.return_value
        mocked_check.validate_requested_check.return_value = True
    ...
    @patch('src.template.missing_journey_code.Check')
    @patch('src.template.missing_journey_code.logger')
    def test_lambda_handler_exception(self, mock_logger, mock_check, mock_observation, mock_get_df_vehicle_journey):
        event = {}
        context = {}
        mocked_check = mock_check.return_value
        mocked_check.set_status = MagicMock()
