...
def test_add_observations():
    ...
    assert len([record for record in added_observations 
                if record.taskresults_id == 1 and
                    record.details == 'No linked values' and
                    record.service_pattern_stop_id is None and
                    record.vehicle_journey_id is None
    ]) == 1
    assert len([record for record in added_observations 
                if record.taskresults_id == 1 and
                    record.details == 'Added Vehicle Journey ID' and
                    record.service_pattern_stop_id is None and
                    record.vehicle_journey_id == 1
    ]) == 1
    assert len([record for record in added_observations 
                if record.taskresults_id == 1 and
                    record.details == 'Added Service Pattern Stop ID' and
                    record.service_pattern_stop_id == 1 and
                    record.vehicle_journey_id is None
    ]) == 1
