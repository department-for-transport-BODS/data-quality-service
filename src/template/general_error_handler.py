...
def lambda_handler(event, context):
    ...
    try:
        ...
        dq_report = dq_report_instance.get_dq_report_by_revision_id(
            revision_id, DQSReportStatus.PIPELINE_PENDING.value
        )
        if dq_report.empty:
        ...
        tasks["status"] = tasks["status"].apply(
            lambda x: (
                DQSTaskResultStatus.FAILED.value
                if x == DQSTaskResultStatus.PENDING.value
                else x
            )
        )
        tasks = tasks[["dataquality_report_id", "status"]].drop_duplicates()
        dq_report_with_status = (
            tasks.groupby(["dataquality_report_id"])[["status"]]
            .apply(map_pipeline_status, include_groups=False)
            .reset_index()
        )
        dq_report_with_status = dq_report_with_status.rename(
            columns={"pipeline_status": "status"}
        )
        dq_report_with_status.rename(
            columns={"dataquality_report_id": "id"}, inplace=True
        )
        dq_report_with_status = (
            dq_report_with_status[["id", "status"]]
            .drop_duplicates()
            .reset_index(drop=True)
        )

        df_generate_csv = dq_report_with_status[
            dq_report_with_status["status"].isin(
                [
