...
def lambda_handler(event, context):
    ...
    dq_reports = dq_report_instance.get_dq_reports_by_status(
        DQSReportStatus.PIPELINE_PENDING.value
    )
    # Overcome issue with Freezegun not freezing date for pd.timestamp.now()
    now = pd.to_datetime(datetime.now(), utc=True)
    timeout_hours = now - pd.Timedelta(
        hours=os.getenv("TIMEOUT_HOURS", Timeouts.TIMEOUT_HOURS.value)
    )
    ...
    if filtered_report_ids:
        ...
        dq_reports_with_status = (
            unique_results.groupby(["dataquality_report_id"])[["status"]]
            .apply(map_pipeline_status, include_groups=False)
            .reset_index()
        )
        dq_reports_with_status.rename(
            columns={"dataquality_report_id": "id"}, inplace=True
        )

    dq_report_timeouts["status"] = DQSReportStatus.TIMEOUT.value
    df_update_dq_reports = pd.concat(
        [dq_reports_with_status, dq_report_timeouts[["id", "status"]]]
    )
    df_update_dq_reports = df_update_dq_reports.rename(
        columns={"pipeline_status": "status"}
    )
    ...
    df_generate_csv = df_update_dq_reports[
        df_update_dq_reports["status"].isin(
            [
                DQSReportStatus.PIPELINE_SUCCEEDED.value,
                DQSReportStatus.PIPELINE_SUCCEEDED_WITH_ERRORS.value,
            ]
        )
    ]
    ...
    try:
        ...
        task_result_instance.update_task_results_status_using_ids(
