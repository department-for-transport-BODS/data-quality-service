...
def map_pipeline_status(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if (df['status'] == DQSTaskResultStatus.SUCCESS.value).all() or DQSTaskResultStatus.DUMMY_SUCCESS.value in status:
    ...
    if (df['status'] == DQSTaskResultStatus.FAILED.value).all() or DQSTaskResultStatus.SUCCESS.value not in status:
